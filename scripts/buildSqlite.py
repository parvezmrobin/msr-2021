...
c.execute('''CREATE TABLE sstubs (
    parent char(40), child char(40), type varchar(64), project varchar, file varchar, line unsigned int
)''')
...
c.execute('CREATE TABLE bugs (parent char(40), child char(40), project varchar, file varchar, line unsigned int)')
c.execute('DROP TABLE IF EXISTS bugs_large')
c.execute('CREATE TABLE bugs_large (parent char(40), child char(40), project varchar, file varchar, line unsigned int)')

# Insert into sstubs
df = pd.read_json('../dataset/sstubs')
values = []
for _, row in df.iterrows():
    value = (row.fixCommitParentSHA1, row.fixCommitSHA1, row.bugType, row.projectName, row.bugFilePath, row.bugLineNum)
    values.append(value)
...
values = []
for _, row in df.iterrows():
    value = (row.fixCommitParentSHA1, row.fixCommitSHA1, row.projectName, row.bugFilePath, row.bugLineNum)
    values.append(value)
